ALTER TABLE public.events ADD COLUMN IF NOT EXISTS theme_color TEXT NOT NULL DEFAULT 'orange';
COMMENT ON COLUMN public.events.theme_color IS 'Preset de cor do hotsite: orange, purple, blue, green';
...
SELECT e.id, e.church_id, c.name AS church_name, e.title, e.slug, e.description,
       e.cover_url, e.location, e.start_date, e.end_date, e.is_open, e.is_published,
       e.cta_text, e.theme_color, e.price, e.is_custom, e.step_personal, e.step_christian_life,
