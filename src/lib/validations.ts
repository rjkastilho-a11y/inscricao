...
export const eventSchema = z.object({
  ...
  show_registration: z.boolean().default(true),
  subtitle: z.string().optional(),
  hotsite_title: z.string().optional(),
  is_open: z.boolean().default(false),
  is_published: z.boolean().default(false),
  cta_text: z.string().default('Garantir Inscrição'),
  theme_color: z.enum(['orange', 'purple', 'blue', 'green']).default('orange'),
  hero_layout: z.enum(['contained', 'full']).default('contained'),
  organizer_name: z.string().optional().or(z.literal('')),
  organizer_logo_url: z.string().url('URL de imagem inválida').optional().or(z.literal('')),
  organizer_whatsapp: z.string().optional().or(z.literal('')),
  max_capacity: z.coerce.number().min(0).optional(),
  price: z.coerce.number().min(0).default(0),
  cover_url: z.string().url('URL de imagem inválida').optional().or(z.literal('')),
